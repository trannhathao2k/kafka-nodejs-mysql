...
// const saslUsername = 'haotn.cmu';
// const saslPassword = 'Cmu#2017';

const producer = new Producer(client);

producer.on('ready', () => {
  console.log('Kafka producer phía khách hàng đang chạy');
});
...
const sendMessage = (payload, topic) => {
  ...
  producer.send(record, (error, result) => {
    ...
    if (error) {
      ...
      console.log(`Payload được gửi từ khách hàng đến Kafka topic ${topic}: ${JSON.stringify(payload)}`);
    }
  });
